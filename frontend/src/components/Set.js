...
const Set = forwardRef(({ setNumber, sets, setSets, removeSet }, ref) => {
    ...
    return (
        <div className="grid grid-flow-col auto-rows-max grid-cols-[1fr,1fr,1fr,1fr,1fr,max-content] gap-x-8 place-content-evenly">
            ...
            <input type="checkbox" id="status" name="status" ref={completion} required className="p-2 h-7 w-7 inline-flex justify-self-center self-center text-center bg-stone-700 my-4" defaultChecked={sets[setNumber - 1]['isFinished'] || false} key={sets[setNumber - 1]['isFinished']}/>
